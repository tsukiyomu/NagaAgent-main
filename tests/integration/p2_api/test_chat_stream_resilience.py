"""Integration resilience tests for P2 `/chat/stream` SSE behavior.
...
class TestChatStreamRouteWithFakeLoop:
    ...
    def test_chat_stream_resilience_baseline_finishes_and_cleans_state(self, stream_env, request):
        """Happy path: stream emits content + round_end + [DONE], then cleans state."""
        stream_env.monkeypatch.setattr(
            stream_env.agentic_tool_loop,
            "run_agentic_loop",
            _make_fake_loop(
                'data: {"type":"content","text":"baseline-ok"}\n\n',
        ...
        assert "baseline-ok" in stream_text

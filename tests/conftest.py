...
_offline_bootstrap = OfflineBootstrap()
_offline_bootstrap.install()
...
def _reject_swallowed_network_attempts():
    before = len(_offline_bootstrap.attempts)
    yield
    unexpected = _offline_bootstrap.attempts[before:]
    assert not unexpected, f"offline test attempted external connections: {unexpected}"


def pytest_sessionfinish(session, exitstatus):
    if _offline_bootstrap.attempts:
        # Caught/swallowed connection errors must not produce a false green run.
        reporter = session.config.pluginmanager.get_plugin("terminalreporter")
        if reporter is not None:
            reporter.write_line(f"OFFLINE VIOLATION: {_offline_bootstrap.attempts}", red=True)
        session.exitstatus = pytest.ExitCode.TESTS_FAILED


def pytest_unconfigure(config):
    _offline_bootstrap.close()

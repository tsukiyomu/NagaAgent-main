...
@contextmanager
def _safe_sdk_context(factory: Callable[[], Any]):
    """Isolate SDK construction/entry/exit errors, never the wrapped business error.

    SDKs often defer work until __enter__ or __exit__. A try/except around only
    the factory cannot protect callers at those boundaries. Cleanup's suppression
    return value is deliberately ignored: telemetry does not own caller errors.
    """
    context = None
    observation = None
    try:
    ...
    try:
        ...
        # Includes cancellation and generator close; always propagate the original.
...
def start_observation(*, name: str, as_type: str = "span", **kwargs: Any):
    """Start a best-effort observation usable with an unconditional `with`."""
...
def shutdown_langfuse() -> None:
    """Flush and close an existing client once; never initialize during cleanup.

    Exceptions are isolated, but these synchronous SDK calls have no adapter-level
    deadline. A future async lifecycle caller must arrange off-loop execution and
    bounded shutdown; catching exceptions alone does not guarantee fast exit.
    """
    global _langfuse_client
    client = _langfuse_client
    if client is None:
        return
    # Keep the initialized flag: a closed client must not be reused/reinitialized.
    _langfuse_client = None
...
def flush_langfuse() -> None:
    """Best-effort synchronous flush of an existing client, without initializing."""
    client = _langfuse_client
    if client is None:
        return
